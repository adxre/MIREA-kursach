...
"""
Script to launch multiple instances of DDoS.
"""
...
LOGGER = logging.getLogger("DDoS_Launcher")
...
def launch_server(args: argparse.Namespace):
    """
    Launch the DDoS server.
    :param args: The arguments.
    :type args: argparse.Namespace
    """
    # pylint: disable=import-outside-toplevel, duplicate-code
    from server.ddos_server import DDoSServer

    msg_args = copy(args)
    msg_args.__dict__.pop('target')
    LOGGER.info(get_live_message("DDoS Server is Live!", msg_args))
    if args.gui:
        if platform.system() == 'Windows':
            from server.logger import WinNamedPipeHandler, UnixNamedPipeHandler

            logging.getLogger('DDoS_Server').addHandler(
...
def launch_client(args: argparse.Namespace):
    ...
    if args.stealth:
        logging.getLogger('DDoS_Client').setLevel(logging.ERROR)
    LOGGER.info(get_live_message("Launching", args))
...
def add_client_parser(subparsers: argparse._SubParsersAction):
    ...
    """
    # pylint: disable=import-outside-toplevel
    from client.ddos import modify_parser

    client_parser = subparsers.add_parser(
        "client",
        description="The DDoS Bot Launcher",
        help="Launches multiple DDoS Clients.\n"
        "(Check out [ddos_launcher.py client -h])"
    )
...
def add_server_parser(subparsers: argparse._SubParsersAction):
    ...
    """
    # pylint: disable=import-outside-toplevel
    from server.ddos_server import modify_parser

    server_parser = subparsers.add_parser(
        "server",
        description="The DDoS Server Launcher",
        help="Launches the DDoS Server.\n"
        "(Check out [ddos_launcher.py server -h])"
    )
