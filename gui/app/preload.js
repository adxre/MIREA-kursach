...
/* harmony export */   "ClientCommands": () => (/* binding */ ClientCommands),
 ...
 */
let ClientCommands = /*#__PURE__*/function (ClientCommands) {
  ClientCommands["ERROR"] = "ClientCommands.ERROR";
  ClientCommands["KILLED"] = "ClientCommands.KILLED";
  ClientCommands["STANDBY"] = "ClientCommands.STANDBY";
  ClientCommands["SEND_TARGET"] = "ClientCommands.SEND_TARGET";
  ClientCommands["READ_STATUS"] = "ClientCommands.READ_STATUS";
 ...
 */
let StatusCodes = /*#__PURE__*/function (StatusCodes) {
  StatusCodes["CONNECTION_FAILURE"] = "StatusCodes.CONNECTION_FAILURE";
  StatusCodes["NO_LUCK"] = "StatusCodes.NO_LUCK";
  StatusCodes["ANTI_DDOS"] = "StatusCodes.ANTI_DDOS";
  StatusCodes["FORBIDDEN"] = "StatusCodes.FORBIDDEN";
  StatusCodes["NOT_FOUND"] = "StatusCodes.NOT_FOUND";
  StatusCodes["PWNED"] = "StatusCodes.PWNED";
  return StatusCodes;
}({});
 ...
 * @property {string} ERROR
 */
let Messages = /*#__PURE__*/function (Messages) {
  Messages["DATA"] = "Data";
 ...
 */
let ErrorMessage = /*#__PURE__*/function (ErrorMessage) {
  ErrorMessage["CONNECTION_ABORTED"] = "ErrorMessages.CONNECTION_ABORTED";
  ErrorMessage["CONNECTION_REFUSED"] = "ErrorMessages.CONNECTION_REFUSED";
  ErrorMessage["CONNECTION_RESET"] = "ErrorMessages.CONNECTION_RESET";
  return ErrorMessage;
}({});
...
/* harmony import */ var _types_enums_d__WEBPACK_IMPORTED_MODULE_7__ = __webpack_require__(/*! ../types/enums.d */ "./types/enums.d.ts");
var _context;
...
/** This module facilitates the communication with DDoS Server
via a Named Pipe. */
...
PIPE_NAME = process.platform === 'win32' ? `\\\\.\\pipe\\${PIPE_NAME}` : `/tmp/${PIPE_NAME}`;

// Regex to extract data from the logs
const botPatten = new RegExp(_babel_runtime_corejs3_core_js_stable_instance_map__WEBPACK_IMPORTED_MODULE_0___default()(_context = [/(?<message>Data|Status|Target|Error){1}.*/, /(?:<(?<data>.*?)>).+/, /\[(?:(?<botIp>.*):(?<botPort>.*))*?\]/]).call(_context, r => r.source).join(''));

// Initialize the botnetData
const botnetData = {
  target: {
    url: '',
    status: _types_enums_d__WEBPACK_IMPORTED_MODULE_7__.StatusCodes.NO_LUCK
  },
  botList: []
};

// The Pipe Monitor function
const monitor = callbacks => {
  ...
  const updateClient = input => {
    ...
    const data = {
      ...
      botPort: _babel_runtime_corejs3_core_js_stable_parse_int__WEBPACK_IMPORTED_MODULE_1___default()(result.groups.botPort)
    };
    // Find the bot in the botnetData if it exists
    const bot = _babel_runtime_corejs3_core_js_stable_instance_find__WEBPACK_IMPORTED_MODULE_2___default()(_context2 = botnetData.botList).call(_context2, b => b.ip === data.botIp && b.port === data.botPort);
    switch (data.message) {
      // If a Send Target message is received, update the target.
      case _types_enums_d__WEBPACK_IMPORTED_MODULE_7__.Messages.TARGET:
        botnetData.target = {
          url: data.data,
          status: _types_enums_d__WEBPACK_IMPORTED_MODULE_7__.StatusCodes.NO_LUCK
        };
      ...
      case _types_enums_d__WEBPACK_IMPORTED_MODULE_7__.Messages.DATA:
        switch (data.data) {
          // Add new bots to the botnetData
          case _types_enums_d__WEBPACK_IMPORTED_MODULE_7__.ClientCommands.SEND_TARGET || _types_enums_d__WEBPACK_IMPORTED_MODULE_7__.ClientCommands.STANDBY:
            if (!bot) {
              botnetData.botList.push({
                ip: data.botIp,
                port: data.botPort,
                status: "online",
                targetStatus: _types_enums_d__WEBPACK_IMPORTED_MODULE_7__.StatusCodes.NO_LUCK
              });
            } else {
              bot.status = "online";
            }
            if (data.data === _types_enums_d__WEBPACK_IMPORTED_MODULE_7__.ClientCommands.SEND_TARGET) {
              botnetData.target.status = _types_enums_d__WEBPACK_IMPORTED_MODULE_7__.StatusCodes.NO_LUCK;
            }
            break;
          // Set the bot's status to offline
          case _types_enums_d__WEBPACK_IMPORTED_MODULE_7__.ClientCommands.KILLED || _types_enums_d__WEBPACK_IMPORTED_MODULE_7__.ClientCommands.ERROR:
            if (bot) {
              bot.status = "offline";
            }
            break;
      ...
      case _types_enums_d__WEBPACK_IMPORTED_MODULE_7__.Messages.STATUS:
        if (botnetData.target.status !== _types_enums_d__WEBPACK_IMPORTED_MODULE_7__.StatusCodes.PWNED) {
          botnetData.target.status = _types_enums_d__WEBPACK_IMPORTED_MODULE_7__.StatusCodes[data.data.replace('StatusCodes.', '')];
        }
        if (bot) {
          bot.targetStatus = _types_enums_d__WEBPACK_IMPORTED_MODULE_7__.StatusCodes[data.data.replace('StatusCodes.', '')];
        }
        break;
      // If a Connection Error message is received, set the bot's status to Offline
      case _types_enums_d__WEBPACK_IMPORTED_MODULE_7__.Messages.ERROR:
        if (bot) {
          bot.status = "offline";
        }
    }
  ...
  const server = net__WEBPACK_IMPORTED_MODULE_5__.createServer(stream => {
    ...
    stream.on('data', c => {
      ...
      // Regex for commands.
      _babel_runtime_corejs3_core_js_stable_instance_for_each__WEBPACK_IMPORTED_MODULE_3___default()(_context3 = _babel_runtime_corejs3_core_js_stable_instance_filter__WEBPACK_IMPORTED_MODULE_4___default()(_context4 = input.split(/\|(.+?)\|/)).call(_context4, Boolean)).call(_context3, updateClient);
      if (callbacks?.onData) {
    ...
    stream.on('end', () => {
      console.log('Connected lost with DDoS Server');
      server.close();
...
electron__WEBPACK_IMPORTED_MODULE_6__.contextBridge.exposeInMainWorld("electronAPI", {
  piper: {
    "monitor": callbacks => {
      monitor(callbacks);
    }
  },
  reload: () => electron__WEBPACK_IMPORTED_MODULE_6__.ipcRenderer.send('request-reload')
});
})();
